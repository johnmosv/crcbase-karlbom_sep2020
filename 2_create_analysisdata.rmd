...
# Add diagdate_scrcr (it is actually already there)
...
```{r add migration}
migrations <- left_merge(pop[, .(lopnr, index_date)], migrations_raw, id_vars = "lopnr")
...
```{r recurrent disease}
first_relapse_date <- scrcr_processed %>%
    group_by(lopnr) %>%
    summarise(relapse_date = min(relapse_loc_date, relapse_mets_date, na.rm = TRUE))

pop <- left_merge(pop, first_relapse_date, by = "lopnr")

relapse_before_index <- truethy(pop$relapse_date < pop$index_date)
if (any(relapse_before_index)) {
    print(glue("Removing {sum(relapse_before_index)} rows with relape before index date"))
